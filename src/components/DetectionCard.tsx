...
class DetectionCard extends React.Component<DetectionCardProps> {
  render() {
    ...
    return (
      <div
        className="flex w-full border-b border-b-tews-mmi-II"
        onClick={this.props.onClick}
      >
        <div
          className={`${intensityColor} w-16 flex items-center justify-center text-2xl font-semibold`}
        >
          {this.props.magnitude?.toFixed(2) || ""}
        </div>

        <div
          className={`p-2 bg-tews-dark flex w-full justify-between text-white whitespace-pre-line hover:bg-tews-black-russian transition-all duration-200 ease-in-out hover:cursor-pointer`}
        >
          ...
          <div className="ml-10">
            ...
            <h5>
              ...
              {this.props.longitude?.toFixed(2) || 0}
            </h5>
          </div>
